...
def normalize_column_names(df):
    """強力校正欄位名稱：解決空白、換行或標題不精準問題"""
    rename_map = {}
    for col in df.columns:
        c_str = str(col).replace(' ', '').replace('\n', '')
        if "姓名" in c_str: rename_map[col] = "姓名"
        elif "申請科別" in c_str or ("科別" in c_str and "備選" not in c_str): rename_map[col] = "科別"
        elif "實習期間" in c_str or "時間" in c_str: rename_map[col] = "實習期間"
...
def smart_read_sheet(file):
    ...
    try:
        ...
        for sn in xls.sheet_names:
            if any(k in sn for k in ["志願", "名單", "工作表4", "實習容額", "時段"]):
                target_sheet = sn
                break
        ...
        for i, row in df_scan.iterrows():
            ...
            if any(k in "".join(row_vals) for k in ["姓名", "科別", "申請科別", "實習期間"]):
                header_idx = i # 修正點：直接使用 i，不要 +1
                break
...
if mode == "醫院代表":
    ...
    with col2: a_file = st.file_uploader("上傳學生志願表", type=['xlsx'], key="a_up")
        ...
        try:
            ...
            if df_q is not None and df_a is not None:
                ...
                if '姓名' not in df_a.columns:
                    st.error("志願表中找不到「姓名」欄位。請檢查 Excel 標題列位置。")
                    st.stop()
